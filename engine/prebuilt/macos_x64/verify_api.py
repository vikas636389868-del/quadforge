#!/usr/bin/env python3
"""
verify_api.py — Load libquadforge.dylib and verify all 14 exported C API functions
on macOS x86_64.

Run this after building to confirm the .dylib matches the api.h contract
expected by bridge.py.  Exits 0 on success, 1 on failure.

Usage:
    python3 QuadForge/engine/prebuilt/macos_x64/verify_api.py
"""
...
def main() -> int:
    ...
    # ---- Load library ----
    print(f"\n=== QuadForge macos_x64 API verification ===\n")
    print(f"Library:  {LIB_PATH}")
    print(f"Machine:  {platform.machine()}")
    print(f"macOS:    {platform.mac_ver()[0]}")

    if not os.path.isfile(LIB_PATH):
        fail("file_exists", f"{LIB_PATH} not found — see MISSING_BINARY.md")
        return 1

    # Architecture check
    machine = platform.machine()
    if machine != "x86_64":
        print(f"\n  NOTE: Running on {machine} — x64 library may work via Rosetta 2")
        print(f"        For native performance on Apple Silicon, use macos_arm64.\n")

    # Pre-load libomp if available (prevents symbol-not-found errors on load)
    _try_preload_libomp()
...
def _try_preload_libomp() -> None:
    """Pre-load libomp so its symbols are available when libquadforge.dylib loads."""
    import ctypes.util
    candidates = [
    ...
    if omp:
        try:
            ctypes.CDLL(omp, mode=ctypes.RTLD_GLOBAL)
            return
        except OSError:
            pass
    for c in candidates:
