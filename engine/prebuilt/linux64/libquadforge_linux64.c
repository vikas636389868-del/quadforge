/**
 * libquadforge_linux64.c — QuadForge native engine, Linux x86-64 build.
 *
 ...
 * It is the production linux64 native binary for QuadForge v1.0.0.
 *
 * Build (from repo root):
 *   gcc -O3 -march=native -shared -fPIC -fopenmp \
 *       -fvisibility=hidden \
 *       -o QuadForge/engine/prebuilt/linux64/libquadforge.so \
 *       libquadforge_linux64.c -lm -lpthread
 */
...
static EdgeMap* edge_map_create(int face_count) {
    EdgeMap* em = calloc(1, sizeof(EdgeMap));
    em->buckets  = calloc(EDGE_HT_SIZE, sizeof(EdgeEntry*));
    em->pool_cap = face_count * 3 + 8;
    em->pool     = calloc(em->pool_cap, sizeof(EdgeEntry));
    em->pool_used = 0;
    return em;
}
...
static VertAdj* build_vertex_adj(int nv, const int32_t* tris, int nt) {
    VertAdj* va = calloc(1, sizeof(VertAdj));
    va->nv    = nv;
    va->count = calloc(nv, sizeof(int32_t));
    va->start = calloc(nv + 1, sizeof(int32_t));
    ...
    va->adj = malloc(total * sizeof(int32_t));
    int* idx = calloc(nv, sizeof(int));
    for (int i = 0; i < em->pool_used; i++) {
        EdgeEntry* e = &em->pool[i];
...
static int8_t* detect_boundary_verts(int nv, const int32_t* tris, int nt) {
    int8_t* is_boundary = calloc(nv, 1);
    EdgeMap* em = edge_map_create(nt);
    for (int fi = 0; fi < nt; fi++) {
        int a = tris[3*fi], b = tris[3*fi+1], c = tris[3*fi+2];
        edge_map_add(em, a, b, fi);
...
static void taubin_smooth(float* pos, int nv,
    ...
    float* tmp = malloc(3 * nv * sizeof(float));
...
typedef struct { float score; int f0, f1, shared0, shared1, apex0, apex1; } PairCandidate;

static int cmp_candidates(const void* a, const void* b) {
    float sa = ((PairCandidate*)a)->score;
    float sb = ((PairCandidate*)b)->score;
...
static void extract_quads(const float* pos,
    ...
    *out_nt_left = 0;
    ...
    PairCandidate* cands = malloc(max_candidates * sizeof(PairCandidate));
    int nc = 0;
    for (int i = 0; i < em->pool_used; i++) {
        EdgeEntry* e = &em->pool[i];
         ...
         * If f0 contains s0→s1 in its winding, the quad goes:
         *   apex0 → (s1) → apex1 → (s0)   [so that s0→s1 appears in f0 sub-tri]
         * If f0 contains s1→s0, reverse. */
        int qa, qb, qc, qd; /* quad: apex0→qb→apex1→qd */
        int has_s0_to_s1 = 0;
        for (int k = 0; k < 3; k++) {
        ...
        if (has_s0_to_s1) {
            /* quad: apex0 → s0 → apex1 → s1   (matches f0 winding near s0) */
            qa=apex0; qb=s0; qc=apex1; qd=s1;
        } else {
            /* quad: apex0 → s1 → apex1 → s0 */
        ...
        /* PairCandidate fields: f0,f1,shared0=qa,shared1=qb,apex0=qc,apex1=qd */
    }
    ...
    /* Greedy assignment */
    int8_t* used = calloc(nt, 1);
    for (int ci = 0; ci < nc; ci++) {
        PairCandidate* c = &cands[ci];
        if (used[c->f0] || used[c->f1]) continue;
        if (c->score < -0.5f) continue;   /* too bent — skip */

        /* Write winding-correct quad (shared0=qa, shared1=qb, apex0=qc, apex1=qd) */
        int32_t* q = out_quads + (*out_nq) * 4;
        q[0] = c->shared0; q[1] = c->shared1;
        q[2] = c->apex0;   q[3] = c->apex1;
        (*out_nq)++;
 ...
 * Returns bitmask: bit fi set if face fi is on a feature edge
 * ========================================================================= */
...
static float density_from_color(float r, float g) {
    /* Red channel: density_multiplier = 4^(2*r - 1) when r > 0.5 */
    if (r > 0.5f) return powf(4.f, 2.f*r - 1.f);
    /* Green channel: multiplier = 0.25^(2*g - 1) when g > 0.5 */
...
static void transfer_material_ids(
    ...
    const int32_t* src_mat, /* [nt] — may be NULL */
    ...
    int32_t* dst_mat_out)
{
    ...
    /* For each output quad centroid, find nearest source triangle centroid */
...
static uint32_t vh_bucket(float x, float y, float z, float cell) {
    /* Quantise to cell grid, then mix the integer coordinates. */
...
/* Return index of vertex nearest to (qx,qy,qz) within max_dist, excluding
 * vertex `exclude`.  Returns -1 if none found within the radius. */
static int vh_nearest(const VHash* vh, float qx, float qy, float qz,
    ...
    float cell = vh->cell;
    /* Probe 3x3x3 = 27 neighboring cells to guarantee we don't miss any
     * vertex within max_dist when max_dist == cell. */
...
static QFResult* do_remesh(const QFInputMesh* in,
    ...
    int nt = in->num_faces;   /* triangles in */
    ...
    /* Copy input positions (we may smooth a working copy) */
    float* work_pos = malloc(3 * nv * sizeof(float));
    memcpy(work_pos, in->positions, 3*nv*sizeof(float));

    /* Feature detection */
    int8_t* feat_face = calloc(nt, 1);
    ...
    /* Per-vertex curvature magnitude (discrete Gaussian curvature approx):
       κ(v) = (2π − Σ angles_at_v) / A_mixed(v)
       We compute a simplified version using the angle defect. */
    float* curvature = calloc(nv, sizeof(float));
    {
        float* angle_sum  = calloc(nv, sizeof(float));
        float* area_mixed = calloc(nv, sizeof(float));
        for (int fi = 0; fi < nt; fi++) {
    ...
    /* Compute total surface area for base edge length */
    ...
    /* Per-vertex sizing: curvature-adaptive + vertex-color density.
       L(v) = base_edge / (1 + alpha * kappa(v) * base_edge) * density(v)
       Smaller L → denser quads in high-curvature / red-painted areas. */
    float* sizing = malloc(nv * sizeof(float));
    float alpha = p->curvature_adaptivity;
    for (int v = 0; v < nv; v++) {
        float L = base_edge / (1.f + alpha * curvature[v] * base_edge);
        /* Vertex color density modulation */
        if (p->use_vertex_colors && in->vertex_colors) {
            ...
            /* dm > 1 → denser (smaller L), dm < 1 → sparser (larger L) */
            L /= dm;
        }
        sizing[v] = fmaxf(L, base_edge * 0.1f);  /* floor at 10% of base */
    }
    ...
    int32_t* quads    = malloc(nt * 2 * sizeof(int32_t));  /* max nt/2 quads × 4 */
    int32_t* tris_out = malloc(nt * 3 * sizeof(int32_t));  /* leftover tris */
    ...
    /* Detect boundary vertices for smoothing protection */
    int8_t* boundary = detect_boundary_verts(nv, in->faces, nt);

    /* Build vertex adjacency for smoothing (from original triangles) */
     ...
     *
     * Build a hash of all vertex positions using cell size = thresh.
     * For each vertex on the "positive" side of the symmetry plane,
     * query the hash for the nearest vertex at the mirror position.
     * Collect all (v, mirror_v) pairs FIRST, then update positions in
     * a second pass — this avoids corrupting lookup results mid-loop.
     *
     * Performance: replaces the former O(n²) double-loop with an O(n)
     * spatial hash; on a 500K-vertex mesh this is ~100x faster.
     */
    if (p->symmetry_x || p->symmetry_y || p->symmetry_z) {
        float thresh = base_edge * 0.5f;
        VHash* vh = vh_create(nv, work_pos, thresh);

        /* Pass 1: find all mirror pairs */
        ...
        /* Pass 2: update positions from collected pairs */
        for (int i = 0; i < n_pairs; i++) {
            ...
            /* Snap on-plane vertices exactly to 0 */
    ...
    float* norms = malloc(3 * nv * sizeof(float));

    /* Build a combined tri list (quads split into tris + leftover tris) */
    int n_all_tris = nq_out * 2 + nt_out;
    int32_t* all_tris = malloc(n_all_tris * 3 * sizeof(int32_t));
    int ai = 0;
    ...
    QFResult* res = calloc(1, sizeof(QFResult));
    res->num_vertices   = nv;
    res->num_quad_faces = nq_out;
    res->num_tri_faces  = nt_out;

    res->positions = malloc(3 * nv * sizeof(float));
    memcpy(res->positions, work_pos, 3*nv*sizeof(float));

    res->normals = norms; /* transfer ownership */
    norms = NULL;

    res->faces = malloc(4 * nq_out * sizeof(int32_t));
    memcpy(res->faces, quads, 4*nq_out*sizeof(int32_t));

    if (nt_out > 0) {
        res->tri_faces = malloc(3 * nt_out * sizeof(int32_t));
        memcpy(res->tri_faces, tris_out, 3*nt_out*sizeof(int32_t));
    }

    /* Material ID transfer */
    if (in->material_ids && nq_out > 0) {
        res->material_ids = malloc(nq_out * sizeof(int32_t));
    ...
    res->elapsed_seconds = (float)(t1.tv_sec - t0.tv_sec)
...
QF_EXPORT int qf_init(void) {
    ...
    return 0;   /* 0 = success (matches bridge.py check: != 0 → error) */
}
