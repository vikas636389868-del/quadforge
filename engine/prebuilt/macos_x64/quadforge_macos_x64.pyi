"""
quadforge_macos_x64.pyi — PEP 561 type stub for loader_macos.py (macos_x64 package).

Provides complete type information for the four public symbols re-exported
from the macos_x64 __init__.py so that static analysers (mypy, Pylance,
Pyright, PyCharm) can fully type-check the macOS Intel x86_64 code path
in bridge.py.
...
def resolve_library_path(hint_file: Optional[str] = None) -> Optional[str]:
    ...
    Note:
        On Apple Silicon running Rosetta 2, the x64 dylib is usable but
        the ``macos_arm64`` build is preferred for performance.
...
def load_library(path: str) -> ctypes.CDLL:
    ...
    Side effects:
      * Pre-loads ``libomp.dylib`` from Homebrew (``/usr/local`` on Intel)
        if available, ensuring OpenMP symbols are ready before the dylib loads.
