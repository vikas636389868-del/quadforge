/**
 * libquadforge_macos_x64.c — QuadForge native engine, macOS x86_64 build.
 *
 ...
 * It is the production macos_x64 native binary for QuadForge v1.0.0.
 *
 * Build (from repo root):
 *   clang -O3 -arch x86_64 -shared -fPIC -Xpreprocessor -fopenmp \
 *       -fvisibility=hidden -mmacosx-version-min=12.0 \
 *       -I$(brew --prefix libomp)/include \
 *       -L$(brew --prefix libomp)/lib -lomp \
 *       -install_name @rpath/libquadforge.dylib \
 *       -o QuadForge/engine/prebuilt/macos_x64/libquadforge.dylib \
...
static EdgeMap* edge_map_create(int face_count) {
    ...
    /* Allocate pool large enough for ALL directed edges (3 per face).
     * For manifold meshes the actual unique edge count is ≤ 3F/2, but we
     * allocate 3F+8 to handle non-manifold inputs without reallocation. */
...
static void edge_map_add(EdgeMap* em, int32_t va, int32_t vb, int32_t fi) {
     ...
     * makes the release build safe too. */
...
static int vh_nearest(const VHash* vh, float qx, float qy, float qz,
    ...
    float cell = vh->cell;
    /* Probe 3×3×3 = 27 neighboring cells to guarantee we don't miss any
